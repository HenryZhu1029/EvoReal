...
from cvrpdata_evo_pomo import CVRPDataEvo as Evo_POMO
from cvrpdata_evo_lehd import CVRPDataEvo as Evo_LEHD
...
def main(cfg):
    ...
    # Initialize Evolutor
    if cfg.model == "pomo":
        cvrp_evolver = Evo_POMO(cfg, root_dir = ROOT_DIR, generator_llm=client, debug_mode=DEBUG_MODE)
    elif cfg.model == "lehd":
        cvrp_evolver = Evo_LEHD(cfg, root_dir = ROOT_DIR, generator_llm=client, debug_mode=DEBUG_MODE)
    else:
        raise NotImplementedError("cfg has no 'model' config!")

    # Run Evolution Process
    elitist_module = cvrp_evolver.evolve()

    json_file_name = f"elitist_{cfg.module_to_evo}.json"
