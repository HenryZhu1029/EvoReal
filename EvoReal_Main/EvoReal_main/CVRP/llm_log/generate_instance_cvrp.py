...
def generate_demands(n, demand_type=None, customer_positions=None):
    ...
    if demand_type == 'Q1':
        ...
        return np.random.randint(5, 11, size=n)
    elif demand_type == 'Q7':
        return np.random.randint(50, 101, size=n)
    else:
...
def generate_single_instance(
    depot_type, customer_type=None, demand_type=None, n: int = 100,log_prefix = None
):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    depot = generate_depot(depot_type)
    #Construct target path
    if log_prefix is not None:
        coord_path = os.path.join(BASE_DIR,"llm_log",f"{log_prefix}_llm_log", "coord_generator.py")
    else:
        coord_path = os.path.join(BASE_DIR,"llm_log","coord_generator.py")
    module_name = "coord_generator"

    # dynamically import coord_generator.py
    spec = importlib.util.spec_from_file_location(module_name, coord_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)

    # access generate_customer_positions function
    generate_customer_positions = getattr(module, "generate_customer_positions")
    
    cust_pos = generate_customer_positions(n, customer_type)
    demands = generate_demands(n, demand_type, cust_pos)
...
def batch_generate_cvrp_instances(num_instances, n: int = 100, log_prefix=None):
    ...
    for _ in range(num_instances):
        ...
        instance = generate_single_instance(depot_type=depot_type, customer_type=None, demand_type=None, n=n, log_prefix=log_prefix)
