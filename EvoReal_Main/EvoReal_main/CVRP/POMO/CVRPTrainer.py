...
class CVRPTrainer:
    def __init__(self,
        ...
        self.val_root_dir = env_params['root_dir']
    ...
    def run(self):
        ...
        # self.loss_list = []
        past_avg_aug_gap = []
        past_avg_non_aug_gap = []

        best_avg_aug_gap, best_epoch, best_avg_non_aug_gap = 0,0,0
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            if all_done or (epoch % model_save_interval) == 0:
                ...
                torch.save(checkpoint_dict, '{}/EvoReal-CVRP-checkpoint-iteration{}-response{}-epoch{}.pt'.format(self.result_folder,
                                                                                                                  self.iteration_idx,
                                                                                                                  self.response_id,
                                                                                                                  epoch))
                ...
                avg_aug_gap, avg_non_aug_gap = self.compute_avg_aug_gap(test_logger_dict)
                past_avg_aug_gap.append(avg_aug_gap)
                past_avg_non_aug_gap.append(avg_non_aug_gap)
                ...
                if best_epoch == 0 or avg_aug_gap < best_avg_aug_gap:
                    best_avg_aug_gap = avg_aug_gap
                    best_avg_non_aug_gap = avg_non_aug_gap
                    best_epoch = epoch
                    self.no_improvement_count = 0
                else:
                    self.no_improvement_count += 1
                    self.logger.info(f'[EarlyStop] No improvement: best avg gap = {best_avg_aug_gap:.4f}%, current = {avg_aug_gap:.4f}%. ({self.no_improvement_count}/{self.tolerance})')

            # ========== whether stop ==========
            if self.early_stop_flag and self.no_improvement_count>=self.tolerance:
                self.logger.info(f"[EarlyStop] Training stopped early due to no improvement for {self.tolerance} epochs!")
                self.logger.info(f'*********The best avg AUG gap found so far is {best_avg_aug_gap}% at epoch {best_epoch}***********')
                break
            
        self.logger.info(" *** All training epochs is Done *** ")
        
        return best_avg_aug_gap, best_avg_non_aug_gap
    ...
    def _run_eval(self, epoch, cuda_device_num=0):
            ...
            print(f"\n[Eval] Running eval: {' '.join(map(str,cmd))}")
            ...
            if os.path.exists(output_path):
                ...
                try:
                    ...
                    print(f"[Eval] Deleted eval json file: {output_path}")
    ...
    def compute_avg_aug_gap(self, test_logger_dict, label_file='./dataset/cvrp_label.json'):
        ...
        for prob, scores in test_logger_dict.items():
            ...
            non_aug_score = scores[0][0]
            opt = label_dict[prob]["normalized_label"]
            aug_gap = (aug_score - opt) / opt * 100
            non_aug_gap = (non_aug_score-opt)/opt*100
            aug_gaps.append(aug_gap)
            non_aug_gaps.append(non_aug_gap)

        if len(aug_gaps) == 0:
            avg_aug_gap = None
            avg_non_aug_gap = None
            ...
            self.logger.info(f"[Eval] The average aug gap of {len(aug_gaps)} problems is {avg_aug_gap:.4f}%.")
        return avg_aug_gap, avg_non_aug_gap
