...
def eval(epoch,val_root_dir,idx_iteration,idx_response_id, checkpoint_path=None, cuda_device_num=0):
    ...
    tester_params = {
        ...
        'test_data_load': {
            'enable': True,
            'pt_path': val_root_dir
        },
    }
    root_dir = tester_params['test_data_load']['pt_path']
    tester_params['result_folder'] = checkpoint_path
...
if __name__ == "__main__":
    ...
    val_root_dir = sys.argv[2]
    idx_iteration = int(sys.argv[3])
    idx_response_id = int(sys.argv[4])
    checkpoint_path = sys.argv[5]
    cuda_device_num = int(sys.argv[6])
    tester_logger_dict = eval(epoch, val_root_dir=val_root_dir, 
