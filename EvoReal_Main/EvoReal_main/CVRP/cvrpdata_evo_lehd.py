...
class CVRPDataEvo:
    def __init__(
        ...
        self.n_proc = cfg.n_proc
        self.obj_type = cfg.problem.obj_type
        self.num_training_problems = cfg.num_training_problems
    ...
    def add_seed_to_population(self, module_type: str=None):
        ...
        if gap is None or not isinstance(gap, float):
            individual.update(
                {
                    "exec_success": False,
                    "gap": float("inf"),
            ...
            logging.info(f"[Seed Run] {module_type}: gap={gap:.4f}")
    ...
    def _run_code(self, individual: dict, response_id: int, module_type: str = None):
        ...
        # check code validity
        if individual.get("code") is None or not isinstance(individual["code"], str):
        ...
        # write code to file
        with open(code_dump_path, 'w') as wf:
            wf.write(individual["code"] + '\n')
        ...
        with open(stdout_path, 'w') as f:
            try:
                ...
                score_output = extract_json_from_stdout(out)
                if score_output is None:
                    f.write("Traceback: Failed to extract valid JSON result from subprocess stdout\n")
                    logging.error("Failed to parse gap from get_fitness_score output")
                    return None, None

                return score_output.get("gap"), np.inf

            except subprocess.TimeoutExpired:
                f.write("Traceback: Timeout after 1 hour.\n")
                logging.error(f"Timeout: Process {response_id} killed after 1 hour.")
                return None, None
            except Exception as e:
    ...
    def evaluate_population(self, population: list[dict], module_type: str=None) -> list[dict]:
        ...
        for individual in population:
            ...
            # Run TRAIN + EVAL
            try:
                gap, _ = self._run_code(individual, response_id, module_type=module_type)
            except Exception as e:
    ...
    def rank_select(self, population: list[dict]) -> list[dict]:
        ...
        population = sorted(population, key=lambda x: x["gap"])
    ...
    def update_elitist(self, module_type: str) -> None:
        ...
        # Get the gap list
        gaps = [ind["gap"] for ind in population if ind.get("exec_success", False)]
        best_gap = min(gaps)
        ...
        # Find the individual with the smallest gap
        best_individual = min(valid_population, key=lambda ind: ind["gap"])
        best_gap = best_individual["gap"]

        # update current elite
        if elitist is None or best_gap < elitist["gap"]:
            setattr(self, f"elitist_{module_type}", best_individual)
            logging.info(f"[{module_type}] Iter {self.iteration}: Elitist updated, gap = {best_gap:.4f}")
