...
class TSPDataEvo:
    def __init__(
        ...
        # self.train_set_dir = train_root_dir
        ...
        self.population_S1, self.population_S2, self.population_S3 = [], [], []
        self.elitist_S1, self.elitist_S2, self.elitist_S3 = None,None,None
        self.best_S1_module,self.best_S2_module,self.best_S3_module = None,None,None
        # self.best_R_path, self.best_C_path, self.best_RC_path = None,None,None
        self.best_S1_gap, self.best_S2_gap, self.best_S3_gap = None,None,None
        self.best_S1_iterID, self.best_S2_iterID, self.best_S3_iterID = 0,0,0


        # Hyperparameter setting
        self.mutation_rate = cfg.mutation_rate

        # Prompt words and reflection content
        self.long_term_reflection_str_S1, self.long_term_reflection_str_S2, self.long_term_reflection_str_S3 = "","",""

        # Early Stopping setup
        self.early_stopping_S1,self.early_stopping_S2,self.early_stopping_S3 = False,False,False
        ...
        if self.model == 'lehd':
            self.num_training_problems = self.cfg.num_training_problems
            self.n_proc = self.cfg.n_proc
            
        if hasattr(cfg, "log_prefix") and self.cfg.log_prefix:
            self.log_prefix = self.cfg.log_prefix
        else:
            self.log_prefix = datetime.now().strftime("%Y%m%d_%H%M%S") + "_"+ self.model+ f"_type{self.cfg.module_to_evo.upper()}"
        # path 
        self.log_save_path = os.path.join(self.root_dir, f"{self.log_prefix}_llm_log", "gpt")
        self.checkpoint_folder = os.path.join(self.log_save_path, "checkpoints")
    ...
    def init_prompt(self) -> None:
        ...
        self.module_S1_path = f"{self.root_dir}/{self.log_prefix}_llm_log/gpt_S1.py"
        self.module_S3_path = f"{self.root_dir}/{self.log_prefix}_llm_log/gpt_S3.py"
        self.module_S2_path = f"{self.root_dir}/{self.log_prefix}_llm_log/gpt_S2.py"
        self.mixed_generator_path = f"{self.root_dir}/{self.log_prefix}_llm_log/mixed_generator.py"
        # Loading all text prompts
        # Problem-specific prompt components
        problem_prompt_path = f'{self.prompt_dir}'
        
        self.func_signature_S1 = file_to_string(f'{problem_prompt_path}/func_signature_S1.txt')
        self.func_signature_S3 = file_to_string(f'{problem_prompt_path}/func_signature_S3.txt')
        self.func_signature_S2 = file_to_string(f'{problem_prompt_path}/func_signature_S2.txt')
        ...
        self.S3_design_guidance = file_to_string(f'{self.prompt_dir}/S3_design_prompt.txt')
        self.S1_design_guidance = file_to_string(f'{self.prompt_dir}/S1_design_prompt.txt')
        self.S2_design_guidance = file_to_string(f'{self.prompt_dir}/S2_design_prompt.txt')
        
        ### Seed Functions
        self.seed_S3 = file_to_string(f'{self.prompt_dir}/seed_S3.txt')
        self.seed_S1 = file_to_string(f'{self.prompt_dir}/seed_S1.txt')
        self.seed_S2 = file_to_string(f'{self.prompt_dir}/seed_S2.txt')
        self.S1_func_name = "generate_s1_type"
        self.S3_func_name = "generate_s3_type"
        self.S2_func_name = "generate_s2_type"
        self.seed_S1_prompt = file_to_string(f'{self.prompt_dir}/seed.txt').format(
            external_guide = self.S1_design_guidance,
            seed_func=self.seed_S1,
            func_name=self.S1_func_name,
        )
        self.seed_S3_prompt = file_to_string(f'{self.prompt_dir}/seed.txt').format(
            external_guide = self.S3_design_guidance,
            seed_func=self.seed_S3,
            func_name=self.S3_func_name,
        )
        self.seed_S2_prompt = file_to_string(f'{self.prompt_dir}/seed.txt').format(
            external_guide = self.S2_design_guidance,
            seed_func=self.seed_S2,
            func_name=self.S2_func_name,
        )
        ...
        self.print_short_term_reflection_prompt = False # Print short-term reflection prompt for the first iteration
        self.print_long_term_reflection_prompt = False # Print long-term reflection prompt for the first iteration
    ...
    def add_seed_to_population(self, module_type: str=None):
        ...
        seed_code = getattr(self, f"seed_{module_type}")
        func_name = getattr(self, f"{module_type}_func_name") 
        file_name = f"{self.log_prefix}_llm_log/gpt/iter{self.iteration}_module{module_type}_resp0.txt"

        with open(file_name, 'w') as f:
            f.write(seed_code + "\n")

        stdout_path = f"{self.log_prefix}_llm_log/gpt/iter{self.iteration}_module{module_type}_resp0_stdout.txt"
        ...
        if aug_gap is None or not isinstance(aug_gap, float):
            ...
            self.debug_run_code_succ = False
            if non_aug_gap is not None:
                logging.info(f"[Seed Run] {module_type}: aug_gap={aug_gap:.4f}, non_aug_gap={non_aug_gap:.4f}")
            else:
                logging.info(f"[Seed Run] {module_type}: gap/fitness_score={aug_gap:.4f}")
    ...
    def response_to_individual(self, response: str, response_id: int, module_type: str=None) -> dict:
        ...
        file_name = f"{self.log_prefix}_llm_log/gpt/iter{self.iteration}_module{module_type}_resp{response_id}.txt" 
        func_name = getattr(self, f"{module_type}_func_name")
        ...
        std_out_filepath = f"{self.log_prefix}_llm_log/gpt/iter{self.iteration}_module{module_type}_resp{response_id}_stdout.txt"
        if code is None:
    ...
    def _run_code(self, individual: dict, response_id: int, module_type: str=None):
        ...
        # self.problem_dir = Path(self.train_set_dir, f"Type{module_type}")
        ...
        with open(stdout_path, 'w') as f:
            try:
                result = subprocess.run(
                    [
                        "python", "-u", "LEHD/get_fitness_score.py",
                        ...
                        "--n_total", str(self.num_training_problems),
                ...
                try:
                    gap_tuple = ast.literal_eval(last_line)
                    aug_gap = float(gap_tuple[0]) if gap_tuple[0] is not None else None
                    non_aug_gap = None
                    return aug_gap, non_aug_gap
                except Exception as e:
                    f.write("Traceback: Failed to parse returned gap from get_fitness_score: {}\n".format(str(e)))
                    logging.error(f"Failed to parse gap from get_fitness_score: {e}")
                    return None, None
    ...
    def long_term_reflection(self, short_term_reflections: list[str], module_type: str) -> None:
        ...
        log_dir = os.path.join(self.root_dir, f"{self.log_prefix}_llm_log", "reflections")
    ...
    def update_elitist(self, module_type: str) -> None:
        ...
        # Update the global optimal individual
    ...
    def evolve(self):
        ...
        if module_type in self.module_list:
            ...
            if self.debug_run_code_succ:
                logging.info('Run Code Failed! Check codes in LEHD!')
                return None
            ...
            for i in range(self.cfg.init_pop_size % num_batches):
                sizes[i] += 1
            for batch_id in range(num_batches):
                batch_size = sizes[batch_id]
        ...
        while self.iteration < self.max_iterations and self.max_code_runs < self.cfg.max_code_runs:
            ...
            # self.module_list = [m for m in self.module_list if not getattr(self, f"early_stopping_{m}", False)]
            # for prefix,module_type in enumerate(self.module_list.copy()):
            if self._module_to_evo == self.module_list[0]:
                id_prefix = 1
            elif self._module_to_evo == self.module_list[1]:
                id_prefix = 2
            else:
                id_prefix = 3
            ...
            if module_type == "S3":
                self.population_S3 = selected_population if elitist is None or any(ind["response_id"] == elitist["response_id"] for ind in selected_population) else [elitist] + selected_population
            elif module_type == "S1":
                self.population_S1 = selected_population if elitist is None or any(ind["response_id"] == elitist["response_id"] for ind in selected_population) else [elitist] + selected_population
            else:
                self.population_S2 = selected_population if elitist is None or any(ind["response_id"] == elitist["response_id"] for ind in selected_population) else [elitist] + selected_population
