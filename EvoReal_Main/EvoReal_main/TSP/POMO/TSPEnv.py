...
def get_data_generator(print_info: bool, module_type: str = "mixed", log_prefix=None, root_dir="."):
    module_map = {
        "S1": ("gpt_S1", "generate_s1_type"),
        "S3": ("gpt_S3", "generate_s3_type"),
        "S2": ("gpt_S2", "generate_s2_type"),
        "mixed": ("mixed_generator", "tsp_data_generator"),
    }
    if module_type not in module_map:
        raise ImportError(f"Unsupported module_type: {module_type}")
    module_file, func_name = module_map[module_type]
    module_path = os.path.join(root_dir, f"{log_prefix}_llm_log", f"{module_file}.py")
...
class TSPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        if self.env_params['trainable'] is not True:
            ...
            data_generator = get_data_generator(self.print_info,
                                                module_type=self.module_type,
                                                log_prefix=self.env_params['log_prefix'])
            self.print_info = False
            self.problems = data_generator(batch_size=batch_size, problem_size=self.problem_size)
