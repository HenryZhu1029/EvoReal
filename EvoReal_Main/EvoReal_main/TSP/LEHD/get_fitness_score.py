...
from utils import multi_gen_tsp
...
def get_fitness_score(
    ...
    if idx_iteration !=0 or idx_response_id !=0:
        data_dir = os.path.join(ROOT_DIR, log_path, "gpt","data", f"iter_{idx_iteration}_resp_{idx_response_id}")
        os.makedirs(data_dir, exist_ok=True)

    if idx_iteration == 0 and idx_response_id == 0:
        data_dir = os.path.join(ROOT_DIR, "LEHD", "data")
        merged_txt = os.path.join(data_dir, f"type_{module_type_upper}_instances_formatted.txt")
    # === 1. batch generation and solve ===
    else:
        merged_txt = multi_gen_tsp.generate_and_solve_tsp_batch(
            output_dir=data_dir,
            n_total=n_total,
    ...
    if obj_type == 'min':
        return (float(np.min(fitness_score)),None)
    else:
        return (float(np.max(fitness_score)),None)
