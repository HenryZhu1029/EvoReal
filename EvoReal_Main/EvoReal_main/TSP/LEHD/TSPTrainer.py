...
from LEHD.TSPModel import TSPModel as Model
from LEHD.eval_longTrain import main_test
from LEHD.TSPEnv import TSPEnv as Env
from LEHD.utils import *
...
class TSPTrainer:
    def __init__(self,
        ...
        # self.result_folder = self.trainer_params['train_data_dir']
        self.checkpoint_save_folder = self.trainer_params['checkpoint_save_folder']
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            if early_stopping_flag:
                self.logger.info(f"Early stopping triggered at epoch {epoch-1}. Best gap: {best_gap:.6f} remains at epoch{best_epoch}")
                break
            self.logger.info('=================================================================')
            ...
            if all_done or (epoch % model_save_interval) == 0:
                ...
                torch.save(checkpoint_dict, '{}/checkpoint-{}.pt'.format(self.checkpoint_save_folder, epoch))

                score_optimal, score_student ,gap = main_test(epoch=epoch, checkpoint_path=self.checkpoint_save_folder, use_RRC=False,gen_type=self.gen_type,
                                                              cuda_device_num=self.trainer_params['cuda_device_num'])

                saved_gap.append([gap])
